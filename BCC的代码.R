...
library(WGCNA)
#DGE analysis
BCC_top_table<- read.delim("GSE6520.top.table.tsv", header = TRUE, sep = "\t")
BCC_top_table$diffexpressed <- "NS"
BCC_top_table$diffexpressed[BCC_top_table$adj.P.Val< 0.05 & BCC_top_table$logFC > 1] <- "Up"
BCC_top_table$diffexpressed[BCC_top_table$adj.P.Val< 0.05 & BCC_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  BCC_top_table %>% 
...
volcano_plot <- ggplot(data=BCC_top_table, 
  ...
  labs(title="Differential Gene Expression in BCC",
       x="log2 Fold Change",
...
print(volcano_plot)
BCC_DEGs <- BCC_top_table[which(abs(BCC_top_table$logFC) > 1 & BCC_top_table$adj.P.Val < 0.05), c(2, 7)]
str(BCC_DEGs)
DEG24 <- unique(BCC_DEGs$Gene.symbol[!is.na(BCC_DEGs$Gene.symbol) & BCC_DEGs$Gene.symbol != ""])
write.csv(DEG24, 
          file = "BCC_DEGs_23个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE6520', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1BCC02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2BCC02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('BCC临床信息全部31.xlsx')
...
pdf("3BCC02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "BCC_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes24 <- intersect(DEG24,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes24)
write.csv(key_genes_df, "BCC_key_genes_0个.csv", row.names = FALSE)