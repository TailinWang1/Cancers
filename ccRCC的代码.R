...
library(WGCNA)
#DGE analysis
ccRCC_top_table<- read.delim("GSE68417.top.table.tsv", header = TRUE, sep = "\t")
ccRCC_top_table$diffexpressed <- "NS"
ccRCC_top_table$diffexpressed[ccRCC_top_table$adj.P.Val< 0.05 & ccRCC_top_table$logFC > 1] <- "Up"
ccRCC_top_table$diffexpressed[ccRCC_top_table$adj.P.Val< 0.05 & ccRCC_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  ccRCC_top_table %>% 
...
volcano_plot <- ggplot(data=ccRCC_top_table, 
  ...
  labs(title="Differential Gene Expression in ccRCC",
       x="log2 Fold Change",
...
print(volcano_plot)
ccRCC_DEGs <- ccRCC_top_table[which(abs(ccRCC_top_table$logFC) > 1 & ccRCC_top_table$adj.P.Val < 0.05), c(2, 7)]
str(ccRCC_DEGs)
DEG26 <- unique(ccRCC_DEGs$Gene.symbol[!is.na(ccRCC_DEGs$Gene.symbol) & ccRCC_DEGs$Gene.symbol != ""])
write.csv(DEG26, 
          file = "ccRCC_DEGs_1687个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE68417', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1ccRCC02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2ccRCC02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('ccRCC临床信息全部49.xlsx')
...
pdf("3ccRCC02Module-trait.pdf",width = 6, height = 12)
...
pinkGenes <- colnames(pink.expr)
write.csv(pinkGenes, "ccRCC_cancer_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes26 <- intersect(DEG26,pinkGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes26)
write.csv(key_genes_df, "ccRCC_key_genes_114个.csv", row.names = FALSE)