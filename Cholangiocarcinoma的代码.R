...
library(WGCNA)
#DGE analysis
Cholangiocarcinoma_top_table<- read.delim("GSE22633.top.table.tsv", header = TRUE, sep = "\t")
Cholangiocarcinoma_top_table$diffexpressed <- "NS"
Cholangiocarcinoma_top_table$diffexpressed[Cholangiocarcinoma_top_table$adj.P.Val< 0.05 & Cholangiocarcinoma_top_table$logFC > 1] <- "Up"
Cholangiocarcinoma_top_table$diffexpressed[Cholangiocarcinoma_top_table$adj.P.Val< 0.05 & Cholangiocarcinoma_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Cholangiocarcinoma_top_table %>% 
...
volcano_plot <- ggplot(data=Cholangiocarcinoma_top_table, 
  ...
  labs(title="Differential Gene Expression in Cholangiocarcinoma",
       x="log2 Fold Change",
...
print(volcano_plot)
Cholangiocarcinoma_DEGs <- Cholangiocarcinoma_top_table[which(abs(Cholangiocarcinoma_top_table$logFC) > 1 & Cholangiocarcinoma_top_table$adj.P.Val < 0.05), c(2, 8)]
str(Cholangiocarcinoma_DEGs)
DEG08 <- unique(Cholangiocarcinoma_DEGs$Gene.symbol[!is.na(Cholangiocarcinoma_DEGs$Gene.symbol) & Cholangiocarcinoma_DEGs$Gene.symbol != ""])
write.csv(DEG08, 
          file = "Cholangiocarcinoma_DEGs_492个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE22633', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1Cholangiocarcinoma02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Cholangiocarcinoma02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Cholangiocarcinoma临床信息全部63.xlsx')
...
pdf("3Cholangiocarcinoma02Module-trait.pdf",width = 6, height = 12)
...
greenyellowGenes <- colnames( greenyellow.expr)
write.csv(greenyellowGenes, "Cholangiocarcinoma_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes08 <- intersect(DEG08,greenyellowGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes08)
write.csv(key_genes_df, "Cholangiocarcinoma_key_genes_195个.csv", row.names = FALSE)
