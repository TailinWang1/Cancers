...
library(WGCNA)
#DGE analysis
Pituitary_gonadotrope_tumor_top_table<- read.delim("GSE26966.top.table.tsv", header = TRUE, sep = "\t")
Pituitary_gonadotrope_tumor_top_table$diffexpressed <- "NS"
Pituitary_gonadotrope_tumor_top_table$diffexpressed[Pituitary_gonadotrope_tumor_top_table$adj.P.Val< 0.05 & Pituitary_gonadotrope_tumor_top_table$logFC > 1] <- "Up"
Pituitary_gonadotrope_tumor_top_table$diffexpressed[Pituitary_gonadotrope_tumor_top_table$adj.P.Val< 0.05 & Pituitary_gonadotrope_tumor_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Pituitary_gonadotrope_tumor_top_table %>% 
...
volcano_plot <- ggplot(data=Pituitary_gonadotrope_tumor_top_table, 
  ...
  labs(title="Differential Gene Expression in Pituitary gonadotrope tumor",
       x="log2 Fold Change",
...
print(volcano_plot)
Pituitary_gonadotrope_tumor_DEGs <- Pituitary_gonadotrope_tumor_top_table[which(abs(Pituitary_gonadotrope_tumor_top_table$logFC) > 1 & Pituitary_gonadotrope_tumor_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Pituitary_gonadotrope_tumor_DEGs)
DEG30 <- unique(Pituitary_gonadotrope_tumor_DEGs$Gene.symbol[!is.na(Pituitary_gonadotrope_tumor_DEGs$Gene.symbol) & Pituitary_gonadotrope_tumor_DEGs$Gene.symbol != ""])
write.csv(DEG30, 
          file = "Pituitary_gonadotrope_tumor_DEGs_3668个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE26966', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1Pituitarygonadotropetumor02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Pituitarygonadotropetumor02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Pituitary gonadotrope tumor临床信息全部23.xlsx')
...
pdf("3Pituitarygonadotropetumor02Module-trait.pdf",width = 6, height = 12)
...
turquoiseGenes <- colnames(turquoise.expr)
write.csv(turquoiseGenes, "Pituitary_gonadotrope_tumor_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes30 <- intersect(DEG30,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes30)
write.csv(key_genes_df, "Pituitary_gonadotrope_tumor_key_genes_1959个.csv", row.names = FALSE)