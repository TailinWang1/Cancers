...
library(WGCNA)
#DGE analysis
HCC_top_table<- read.delim("GSE1898.top.table.tsv", header = TRUE, sep = "\t")
HCC_top_table$diffexpressed <- "NS"
HCC_top_table$diffexpressed[HCC_top_table$adj.P.Val< 0.05 & HCC_top_table$logFC > 1] <- "Up"
HCC_top_table$diffexpressed[HCC_top_table$adj.P.Val< 0.05 & HCC_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  HCC_top_table %>% 
...
volcano_plot <- ggplot(data=HCC_top_table, 
  ...
  labs(title="Differential Gene Expression in HCC",
       x="log2 Fold Change",
...
print(volcano_plot)
HCC_DEGs <- HCC_top_table[which(abs(HCC_top_table$logFC) > 1 & HCC_top_table$adj.P.Val < 0.05), c(2, 7)]
str(HCC_DEGs)
DEG16 <- unique(HCC_DEGs$Gene.symbol[!is.na(HCC_DEGs$Gene.symbol) & HCC_DEGs$Gene.symbol != ""])
write.csv(DEG16, 
          file = "HCC_DEGs_2118个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE1898', destdir=".",AnnotGPL = T,getGPL = T)
...
plot(sampleTree, main = "Sample clustering to detect outliers", sub="", xlab="", cex.lab = 1.5, cex.axis = 1.5, cex.main = 2) +
  ...
  abline(h = 1500000000, col = "red") 
clust = cutreeStatic(sampleTree, cutHeight = 150, minSize = 0)
keepSamples = (clust==1)
...
pdf("1HCC02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2HCC02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('HCC临床信息全部182.xlsx')
...
pdf("3HCC02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "HCC_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes16 <- intersect(DEG16,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes16)
write.csv(key_genes_df, "HCC_key_genes_1925个.csv", row.names = FALSE)