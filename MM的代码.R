...
library(WGCNA)
#DGE analysis
MM_top_table<- read.delim("GSE29370.top.table.tsv", header = TRUE, sep = "\t")
MM_top_table$diffexpressed <- "NS"
MM_top_table$diffexpressed[MM_top_table$adj.P.Val< 0.05 & MM_top_table$logFC > 1] <- "Up"
MM_top_table$diffexpressed[MM_top_table$adj.P.Val< 0.05 & MM_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  MM_top_table %>% 
...
volcano_plot <- ggplot(data=MM_top_table, 
  ...
  labs(title="Differential Gene Expression in MM",
       x="log2 Fold Change",
...
print(volcano_plot)
MM_DEGs <- MM_top_table[which(abs(MM_top_table$logFC) > 1 & MM_top_table$adj.P.Val < 0.05), c(2, 7)]
str(MM_DEGs)
DEG20 <- unique(MM_DEGs$Gene.symbol[!is.na(MM_DEGs$Gene.symbol) & MM_DEGs$Gene.symbol != ""])
write.csv(DEG20, 
          file = "MM_DEGs_47个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE29370', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1MM02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2MM02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('MM临床信息全部22.xlsx')
...
pdf("3MM02Module-trait.pdf",width = 6, height = 12)
...
greenGenes <- colnames(green.expr)
write.csv(greenGenes, "MM_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes20 <- intersect(DEG20,greenGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes20)
write.csv(key_genes_df, "MM_key_genes_36个.csv", row.names = FALSE)