...
library(WGCNA)
#DGE analysis
Tumor_top_table<- read.delim("GSE65144.top.table.tsv", header = TRUE, sep = "\t")
...
str(Tumor_DEGs)
DEG37 <- unique(Tumor_DEGs$Gene.symbol[!is.na(Tumor_DEGs$Gene.symbol) & Tumor_DEGs$Gene.symbol != ""])
write.csv(DEG37, 
          file = "ATC_DEGs_4645个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE65144', destdir=".",AnnotGPL = T,getGPL = T)
...
T1Dsamples <- read_excel('ATC临床信息全部25.xlsx')
...
write.csv(turquoiseGenes, "ATC_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes37<- intersect(DEG37,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes37)
write.csv(key_genes_df, "ATC_key_genes_727个.csv", row.names = FALSE)
