...
library(WGCNA)
#DGE analysis
CML_top_table<- read.delim("GSE5550.top.table.tsv", header = TRUE, sep = "\t")
CML_top_table$diffexpressed <- "NS"
CML_top_table$diffexpressed[CML_top_table$adj.P.Val< 0.05 & CML_top_table$logFC > 1] <- "Up"
CML_top_table$diffexpressed[CML_top_table$adj.P.Val< 0.05 & CML_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  CML_top_table %>% 
...
volcano_plot <- ggplot(data=CML_top_table, 
  ...
  labs(title="Differential Gene Expression in CML",
       x="log2 Fold Change",
...
print(volcano_plot)
CML_DEGs <- CML_top_table[which(abs(CML_top_table$logFC) > 1 & CML_top_table$adj.P.Val < 0.05), c(2, 7)]
str(CML_DEGs)
DEG07 <- unique(CML_DEGs$Gene.symbol[!is.na(CML_DEGs$Gene.symbol) & CML_DEGs$Gene.symbol != ""])
write.csv(DEG07, 
          file = "CML_DEGs_53个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE5550', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1CML02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2CML02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('CML临床信息全部17.xlsx')
...
pdf("3CML02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "CML_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes07 <- intersect(DEG07,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes07)
write.csv(key_genes_df, "CML_key_genes_33个.csv", row.names = FALSE)