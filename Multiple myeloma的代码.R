...
volcano_plot <- ggplot(data=Multiple_myeloma_top_table, 
  ...
  labs(title="Differential Gene Expression in Multiple myeloma",
       x="log2 Fold Change",
...
print(volcano_plot)
Multiple_myeloma_DEGs <- Multiple_myeloma_top_table[which(abs(Multiple_myeloma_top_table$logFC) > 1 & Multiple_myeloma_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Multiple_myeloma_DEGs)
DEG21 <- unique(Multiple_myeloma_DEGs$Gene.symbol[!is.na(Multiple_myeloma_DEGs$Gene.symbol) & Multiple_myeloma_DEGs$Gene.symbol != ""])
write.csv(DEG21, 
          file = "Multiple_myeloma_DEGs_102个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
...
exclude_samples <- c("GSM341932", "GSM341933", "GSM341934", "GSM341935", "GSM341936", 
                     "GSM341937", "GSM341938", "GSM341939", "GSM341940", "GSM341941", 
                     "GSM341942", "GSM342076", "GSM342077", "GSM342078", "GSM342079", 
                     "GSM342080", "GSM342081", "GSM342082", "GSM342083", "GSM342084")
T1D_datExpr02 <- T1D_datExpr02[, !colnames(T1D_datExpr02) %in% exclude_samples]
...
pdf("1Multiplemyeloma02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Multiplemyeloma02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Multiple myeloma临床信息全部138.xlsx')
...
pdf("3Multiplemyeloma02Module-trait.pdf",width = 6, height = 12)
...
cyanGenes <- colnames(cyan.expr)
write.csv(cyanGenes, "Multiple_myeloma_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes21 <- intersect(DEG21,cyanGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes21)
write.csv(key_genes_df, "Multiple_myeloma_key_genes_35个.csv", row.names = FALSE)
