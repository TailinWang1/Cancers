...
library(WGCNA)
#DGE analysis
Colorectal_adenoma_top_table<- read.delim("GSE8671.top.table.tsv", header = TRUE, sep = "\t")
Colorectal_adenoma_top_table$diffexpressed <- "NS"
Colorectal_adenoma_top_table$diffexpressed[Colorectal_adenoma_top_table$adj.P.Val< 0.05 & Colorectal_adenoma_top_table$logFC > 1] <- "Up"
Colorectal_adenoma_top_table$diffexpressed[Colorectal_adenoma_top_table$adj.P.Val< 0.05 & Colorectal_adenoma_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Colorectal_adenoma_top_table %>% 
...
volcano_plot <- ggplot(data=Colorectal_adenoma_top_table, 
  ...
  labs(title="Differential Gene Expression in Colorectal adenoma",
       x="log2 Fold Change",
...
print(volcano_plot)
Colorectal_adenoma_DEGs <- Colorectal_adenoma_top_table[which(abs(Colorectal_adenoma_top_table$logFC) > 1 & Colorectal_adenoma_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Colorectal_adenoma_DEGs)
DEG32 <- unique(Colorectal_adenoma_DEGs$Gene.symbol[!is.na(Colorectal_adenoma_DEGs$Gene.symbol) & Colorectal_adenoma_DEGs$Gene.symbol != ""])
write.csv(DEG32, 
          file = "Colorectal_adenoma_DEGs_3398个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE8671', destdir=".",AnnotGPL = T,getGPL = T)
...
cutoff = quantile(m.vars, prob = 0.6)
...
pdf("1Colorectaladenoma02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Colorectaladenoma02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Colorectal adenoma临床信息全部64.xlsx')
...
pdf("3Colorectaladenoma02Module-trait.pdf",width = 6, height = 12)
...
write.csv(blueGenes, "Colorectal_adenoma_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes32<- intersect(DEG32,blueGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes32)
write.csv(key_genes_df, "Colorectal_adenoma_key_genes_1052个.csv", row.names = FALSE)