...
library(WGCNA)
#DGE analysis
Melanoma_top_table<- read.delim("GSE15605.top.table.tsv", header = TRUE, sep = "\t")
Melanoma_top_table$diffexpressed <- "NS"
Melanoma_top_table$diffexpressed[Melanoma_top_table$adj.P.Val< 0.05 & Melanoma_top_table$logFC > 1] <- "Up"
Melanoma_top_table$diffexpressed[Melanoma_top_table$adj.P.Val< 0.05 & Melanoma_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Melanoma_top_table %>% 
...
volcano_plot <- ggplot(data=Melanoma_top_table, 
  ...
  labs(title="Differential Gene Expression in Melanoma",
       x="log2 Fold Change",
...
print(volcano_plot)
Melanoma_DEGs <- Melanoma_top_table[which(abs(Melanoma_top_table$logFC) > 1 & Melanoma_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Melanoma_DEGs)
DEG19 <- unique(Melanoma_DEGs$Gene.symbol[!is.na(Melanoma_DEGs$Gene.symbol) & Melanoma_DEGs$Gene.symbol != ""])
write.csv(DEG19, 
          file = "Melanoma_DEGs_3649个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE15605', destdir=".",AnnotGPL = T,getGPL = T)
...
cutoff = quantile(m.vars, prob = 0.7)
...
pdf("1Melanoma02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Melanoma02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Melanoma临床信息全部74.xlsx')
...
pdf("3Melanoma02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "Melanoma_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes19 <- intersect(DEG19,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes19)
write.csv(key_genes_df, "Melanoma_key_genes_2252个.csv", row.names = FALSE)
