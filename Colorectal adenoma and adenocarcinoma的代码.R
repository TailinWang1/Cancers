...
library(WGCNA)
#DGE analysis
Tumor_top_table<- read.delim("GSE41657.top.table.tsv", header = TRUE, sep = "\t")
Tumor_top_table$diffexpressed <- "NS"
Tumor_top_table$diffexpressed[Tumor_top_table$adj.P.Val< 0.05 & Tumor_top_table$logFC > 1] <- "Up"
Tumor_top_table$diffexpressed[Tumor_top_table$adj.P.Val< 0.05 & Tumor_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Tumor_top_table %>% 
...
volcano_plot <- ggplot(data=Tumor_top_table, 
  ...
  labs(title="Differential Gene Expression in Tumor",
       x="log2 Fold Change",
...
print(volcano_plot)
Tumor_DEGs <- Tumor_top_table[which(abs(Tumor_top_table$logFC) > 1 & Tumor_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Tumor_DEGs)
DEG36 <- unique(Tumor_DEGs$Gene.symbol[!is.na(Tumor_DEGs$Gene.symbol) & Tumor_DEGs$Gene.symbol != ""])
write.csv(DEG36, 
          file = "Colorectal_adenoma_and_adenocarcinoma_DEGs_3508个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE41657', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1Tumor02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Tumor02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Colorectal adenoma and adenocarcinoma临床信息全部88.xlsx')
...
pdf("3Tumor02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "Colorectal_adenoma_and_adenocarcinoma_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes36<- intersect(DEG36,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes36)
write.csv(key_genes_df, "Colorectal_adenoma_and_adenocarcinoma_key_genes_650个.csv", row.names = FALSE)
