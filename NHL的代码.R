...
library(WGCNA)
#DGE analysis
NHL_top_table<- read.delim("GSE32018.top.table.tsv", header = TRUE, sep = "\t")
NHL_top_table$diffexpressed <- "NS"
NHL_top_table$diffexpressed[NHL_top_table$adj.P.Val< 0.05 & NHL_top_table$logFC > 1] <- "Up"
NHL_top_table$diffexpressed[NHL_top_table$adj.P.Val< 0.05 & NHL_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  NHL_top_table %>% 
...
volcano_plot <- ggplot(data=NHL_top_table, 
  ...
  labs(title="Differential Gene Expression in NHL",
       x="log2 Fold Change",
...
NHL_DEGs <- NHL_top_table[which(abs(NHL_top_table$logFC) > 1 & NHL_top_table$adj.P.Val < 0.05), c(2, 7)]
str(NHL_DEGs)
DEG14 <- unique(NHL_DEGs$Gene.symbol[!is.na(NHL_DEGs$Gene.symbol) & NHL_DEGs$Gene.symbol != ""])
write.csv(DEG14, 
          file = "NHL_DEGs_329个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE32018', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1NHL02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2NHL02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('NHL临床信息全部127.xlsx')
...
pdf("3NHL02Module-trait.pdf",width = 6, height = 12)
...
purpleGenes <- colnames(purple.expr)
write.csv(purpleGenes, "NHL_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes14 <- intersect(DEG14,purpleGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes14)
write.csv(key_genes_df, "NHL_key_genes_85个.csv", row.names = FALSE)