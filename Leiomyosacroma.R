...
volcano_plot <- ggplot(data=Leiomyosarcoma_top_table, 
  ...
  labs(title="Differential Gene Expression in Leiomyosarcoma",
       x="log2 Fold Change",
...
Leiomyosarcoma_DEGs <- Leiomyosarcoma_top_table[which(abs(Leiomyosarcoma_top_table$logFC) > 1 & Leiomyosarcoma_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Leiomyosarcoma_DEGs)
DEG10 <- unique(Leiomyosarcoma_DEGs$Gene.symbol[!is.na(Leiomyosarcoma_DEGs$Gene.symbol) & Leiomyosarcoma_DEGs$Gene.symbol != ""])
write.csv(DEG10, 
          file = "Leiomyosarcoma_DEGs_524个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
...
pdf("1Leiomyosarcoma02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Leiomyosarcoma02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Leiomyosarcoma临床信息全部79.xlsx')
...
pdf("3Leiomyosarcoma02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "Leiomyosarcoma_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes10 <- intersect(DEG10,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes10)
write.csv(key_genes_df, "Leiomyosarcoma_key_genes_435个.csv", row.names = FALSE)
