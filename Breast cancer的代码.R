...
volcano_plot <- ggplot(data=Breast_cancer_top_table, 
  ...
  labs(title="Differential Gene Expression in Breast cancer",
       x="log2 Fold Change",
...
print(volcano_plot)
Breast_cancer_DEGs <- Breast_cancer_top_table[which(abs(Breast_cancer_top_table$logFC) > 1 & Breast_cancer_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Breast_cancer_DEGs)
DEG03 <- unique(Breast_cancer_DEGs$Gene.symbol[!is.na(Breast_cancer_DEGs$Gene.symbol) & Breast_cancer_DEGs$Gene.symbol != ""])
write.csv(DEG03, 
          file = "Breast_cancer_DEGs_3060个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
...
pdf("1Breast02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Breas02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Breast cancer临床信息全部66.xlsx')
...
pdf("3Breas02Module-trait.pdf",width = 6, height = 12)
...
write.csv(greenGenes, "Breast_cancer_WGVNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#combine DGE and WGCNA
kGenes03 <- intersect(DEG03,greenGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes03)
write.csv(key_genes_df, "Breast_cancer_key_genes_448个.csv", row.names = FALSE)