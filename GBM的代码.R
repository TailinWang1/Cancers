...
library(WGCNA)
#DGE analysis
GBM_top_table<- read.delim("GSE14805.top.table.tsv", header = TRUE, sep = "\t")
GBM_top_table$diffexpressed <- "NS"
GBM_top_table$diffexpressed[GBM_top_table$adj.P.Val< 0.05 & GBM_top_table$logFC > 1] <- "Up"
GBM_top_table$diffexpressed[GBM_top_table$adj.P.Val< 0.05 & GBM_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  GBM_top_table %>% 
...
volcano_plot <- ggplot(data=GBM_top_table, 
  ...
  labs(title="Differential Gene Expression in GBM",
       x="log2 Fold Change",
...
print(volcano_plot)
GBM_DEGs <- GBM_top_table[which(abs(GBM_top_table$logFC) > 1 & GBM_top_table$adj.P.Val < 0.05), c(2, 7)]
str(GBM_DEGs)
DEG27 <- unique(GBM_DEGs$Gene.symbol[!is.na(GBM_DEGs$Gene.symbol) & GBM_DEGs$Gene.symbol != ""])
write.csv(DEG27, 
          file = "GBM_DEGs_3464个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE14805', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1GBM02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2GBM02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('GBM临床信息全部38.xlsx')
...
pdf("3GBM02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "GBM_cancer_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes27 <- intersect(DEG27,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes27)
write.csv(key_genes_df, "GBM_key_genes_2726个.csv", row.names = FALSE)