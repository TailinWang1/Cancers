...
library(WGCNA)
#DGE analysis
PDAC_top_table<- read.delim("GSE15471.top.table.tsv", header = TRUE, sep = "\t")
PDAC_top_table$diffexpressed <- "NS"
PDAC_top_table$diffexpressed[PDAC_top_table$adj.P.Val< 0.05 & PDAC_top_table$logFC > 1] <- "Up"
PDAC_top_table$diffexpressed[PDAC_top_table$adj.P.Val< 0.05 & PDAC_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  PDAC_top_table %>% 
...
volcano_plot <- ggplot(data=PDAC_top_table, 
  ...
  labs(title="Differential Gene Expression in PDAC",
       x="log2 Fold Change",
...
print(volcano_plot)
PDAC_DEGs <- PDAC_top_table[which(abs(PDAC_top_table$logFC) > 1 & PDAC_top_table$adj.P.Val < 0.05), c(2, 7)]
str(PDAC_DEGs)
DEG35 <- unique(PDAC_DEGs$Gene.symbol[!is.na(PDAC_DEGs$Gene.symbol) & PDAC_DEGs$Gene.symbol != ""])
write.csv(DEG35, 
          file = "PDAC_DEGs_1793个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE15471', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1PDAC02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2PDAC02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('PDAC临床信息全部78.xlsx')
...
pdf("3PDAC02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "PDAC_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes35<- intersect(DEG35,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes35)
write.csv(key_genes_df, "PDAC_key_genes_1371个.csv", row.names = FALSE)