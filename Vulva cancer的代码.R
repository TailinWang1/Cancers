...
library(WGCNA)
#DGE analysis
Tumor_top_table<- read.delim("GSE38228.top.table.tsv", header = TRUE, sep = "\t")
...
Tumor_DEGs <- Tumor_top_table[which(abs(Tumor_top_table$logFC) > 1 & Tumor_top_table$adj.P.Val < 0.05), c(2, 8)]
str(Tumor_DEGs)
DEG39 <- unique(Tumor_DEGs$Gene.symbol[!is.na(Tumor_DEGs$Gene.symbol) & Tumor_DEGs$Gene.symbol != ""])
write.csv(DEG39, 
          file = "Vulva_cancer_DEGs_867个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE38228', destdir=".",AnnotGPL = T,getGPL = T)
...
cutoff = quantile(m.vars, prob = 0.95)
...
T1Dsamples <- read_excel('Vulva cancer临床信息全部18.xlsx')
...
write.csv(turquoiseGenes, "Vulva_cancer_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes39<- intersect(DEG39,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes39)
write.csv(key_genes_df, "Vulva_cancer_key_genes_355个.csv", row.names = FALSE)
