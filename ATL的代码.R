...
library(WGCNA)
#DGE analysis
Tumor_top_table<- read.delim("GSE14317.top.table.tsv", header = TRUE, sep = "\t")
...
str(Tumor_DEGs)
DEG40 <- unique(Tumor_DEGs$Gene.symbol[!is.na(Tumor_DEGs$Gene.symbol) & Tumor_DEGs$Gene.symbol != ""])
write.csv(DEG40, 
          file = "ATL_cancer_DEGs_2039个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE14317', destdir=".",AnnotGPL = T,getGPL = T)
...
cutoff = quantile(m.vars, prob = 0.85)
...
T1Dsamples <- read_excel('ATL临床信息全部26.xlsx')
...
write.csv(blueGenes, "ATL_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes40<- intersect(DEG40,blueGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes40)
write.csv(key_genes_df, "ATL_key_genes_198个.csv", row.names = FALSE)
