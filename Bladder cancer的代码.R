...
library(WGCNA)
#DGE analysis
Bladder_cancer_top_table<- read.delim("GSE37815.top.table.tsv", header = TRUE, sep = "\t")
Bladder_cancer_top_table$diffexpressed <- "NS"
Bladder_cancer_top_table$diffexpressed[Bladder_cancer_top_table$adj.P.Val < 0.05 & Bladder_cancer_top_table$logFC > 1] <- "Up"
Bladder_cancer_top_table$diffexpressed[Bladder_cancer_top_table$adj.P.Val< 0.05 & Bladder_cancer_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Bladder_cancer_top_table %>% 
...
volcano_plot <- ggplot(data=Bladder_cancer_top_table, 
  ...
  labs(title="Differential Gene Expression in Bladder cancer",
       x="log2 Fold Change",
...
print(volcano_plot)
Bladder_cancer_DEGs <- Bladder_cancer_top_table[which(abs(Bladder_cancer_top_table$logFC) > 1 & Bladder_cancer_top_table$adj.P.Val < 0.05), c(2, 8)]
str(Bladder_cancer_DEGs)
DEG02 <- unique(Bladder_cancer_DEGs$Gene.symbol[!is.na(Bladder_cancer_DEGs$Gene.symbol) & Bladder_cancer_DEGs$Gene.symbol != ""])
write.csv(DEG02, 
          file = "Bladder_cancer_DEGs_668个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE37815', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1Bladder02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Bladder02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Bladder cancer临床信息全部24.xlsx')
...
pdf("3Bladder02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "Bladder_cancer_WGVNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DGE and WGCNA
kGenes02 <- intersect(DEG02,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes02)
write.csv(key_genes_df, "Bladder_cancer_key_genes_472个.csv", row.names = FALSE)
