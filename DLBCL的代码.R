...
library(WGCNA)
#DGE analysis
DLBCL_top_table<- read.delim("GSE3892.top.table.tsv", header = TRUE, sep = "\t")
DLBCL_top_table$diffexpressed <- "NS"
DLBCL_top_table$diffexpressed[DLBCL_top_table$adj.P.Val< 0.05 & DLBCL_top_table$logFC > 1] <- "Up"
DLBCL_top_table$diffexpressed[DLBCL_top_table$adj.P.Val< 0.05 & DLBCL_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  DLBCL_top_table %>% 
...
volcano_plot <- ggplot(data=DLBCL_top_table, 
  ...
  labs(title="Differential Gene Expression in DLBCL",
       x="log2 Fold Change",
...
DLBCL_DEGs <- DLBCL_top_table[which(abs(DLBCL_top_table$logFC) > 1 & DLBCL_top_table$adj.P.Val < 0.05), c(2, 7)]
str(DLBCL_DEGs)
DEG13 <- unique(DLBCL_DEGs$Gene.symbol[!is.na(DLBCL_DEGs$Gene.symbol) & DLBCL_DEGs$Gene.symbol != ""])
write.csv(DEG13, 
          file = "DLBCL_DEGs_130个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE3892', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1DLBCL02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2DLBCL02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('DLBCL临床信息全部54.xlsx')
...
pdf("3DLBCL02Module-trait.pdf",width = 6, height = 12)
...
blackGenes <- colnames(black.expr)
write.csv(blackGenes, "DLBCL_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes13 <- intersect(DEG13,blackGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes13)
write.csv(key_genes_df, "DLBCL_key_genes_63个.csv", row.names = FALSE)