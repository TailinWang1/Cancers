...
volcano_plot <- ggplot(data=Gastric_cancer_top_table, 
  ...
  labs(title="Differential Gene Expression in Gastric cancer",
       x="log2 Fold Change",
...
Gastric_cancer_DEGs <- Gastric_cancer_top_table[which(abs(Gastric_cancer_top_table$logFC) > 1 & Gastric_cancer_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Gastric_cancer_DEGs)
DEG12 <- unique(Gastric_cancer_DEGs$Gene.symbol[!is.na(Gastric_cancer_DEGs$Gene.symbol) & Gastric_cancer_DEGs$Gene.symbol != ""])
write.csv(DEG12, 
          file = "Gastric_cancer_DEGs_665个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
...
pdf("1Gastric_cancer02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Gastric_cancer02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Gastric cancer临床信息全部30.xlsx')
...
pdf("3Gastriccancer02Module-trait.pdf",width = 6, height = 12)
...
write.csv(blueGenes, "Gastric_cancer_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes12 <- intersect(DEG12,blueGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes12)
write.csv(key_genes_df, "Gastric_cancer_key_genes_104个.csv", row.names = FALSE)
