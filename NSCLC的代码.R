...
library(WGCNA)
#DGE analysis
NSCLC_top_table<- read.delim("GSE19188.top.table.tsv", header = TRUE, sep = "\t")
NSCLC_top_table$diffexpressed <- "NS"
NSCLC_top_table$diffexpressed[NSCLC_top_table$adj.P.Val< 0.05 & NSCLC_top_table$logFC > 1] <- "Up"
NSCLC_top_table$diffexpressed[NSCLC_top_table$adj.P.Val< 0.05 & NSCLC_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  NSCLC_top_table %>% 
...
volcano_plot <- ggplot(data=NSCLC_top_table, 
  ...
  labs(title="Differential Gene Expression in NSCLC",
       x="log2 Fold Change",
...
print(volcano_plot)
NSCLC_DEGs <- NSCLC_top_table[which(abs(NSCLC_top_table$logFC) > 1 & NSCLC_top_table$adj.P.Val < 0.05), c(2, 7)]
str(NSCLC_DEGs)
DEG33 <- unique(NSCLC_DEGs$Gene.symbol[!is.na(NSCLC_DEGs$Gene.symbol) & NSCLC_DEGs$Gene.symbol != ""])
write.csv(DEG33, 
          file = "NSCLC_DEGs_2523个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE19188', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1NSCLC02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
T1Dsamples <- read_excel('NSCLC临床信息全部156.xlsx')
...
pdf("3NSCLC02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "NSCLC_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes33<- intersect(DEG33,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes33)
write.csv(key_genes_df, "NSCLC_key_genes_1241个.csv", row.names = FALSE)
