...
library(WGCNA)
#DGE analysis
HNSCC_top_table<- read.delim("GSE686.top.table.tsv", header = TRUE, sep = "\t")
HNSCC_top_table$diffexpressed <- "NS"
HNSCC_top_table$diffexpressed[HNSCC_top_table$adj.P.Val< 0.05 & HNSCC_top_table$logFC > 1] <- "Up"
HNSCC_top_table$diffexpressed[HNSCC_top_table$adj.P.Val< 0.05 & HNSCC_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  HNSCC_top_table %>% 
...
volcano_plot <- ggplot(data=HNSCC_top_table, 
  ...
  labs(title="Differential Gene Expression in HNSCC",
       x="log2 Fold Change",
...
print(volcano_plot)
HNSCC_DEGs <- HNSCC_top_table[which(abs(HNSCC_top_table$logFC) > 1 & HNSCC_top_table$adj.P.Val < 0.05), c(2, 7)]
str(HNSCC_DEGs)
DEG18 <- unique(HNSCC_DEGs$Gene.symbol[!is.na(HNSCC_DEGs$Gene.symbol) & HNSCC_DEGs$Gene.symbol != ""])
write.csv(DEG18, 
          file = "HNSCC_DEGs_11个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE686', destdir=".",AnnotGPL = T,getGPL = T)
...
m.vars = apply(datExpr0, 2, var)
var_cutoff = quantile(m.vars, 0.20) 
...
pdf("1HNSCC02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2HNSCC02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('HNSCC临床信息全部78.xlsx')
...
pdf("3HNSCC02Module-trait.pdf",width = 6, height = 12)
...
write.csv(brownGenes, "HNSCC_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes18 <- intersect(DEG18,brownGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes18)
write.csv(key_genes_df, "HNSCC_key_genes_2个.csv", row.names = FALSE)