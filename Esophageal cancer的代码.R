...
library(WGCNA)
#DGE analysis
Esophageal_cancer_top_table<- read.delim("GSE9982.top.table.tsv", header = TRUE, sep = "\t")
Esophageal_cancer_top_table$diffexpressed <- "NS"
Esophageal_cancer_top_table$diffexpressed[Esophageal_cancer_top_table$adj.P.Val< 0.05 & Esophageal_cancer_top_table$logFC > 1] <- "Up"
Esophageal_cancer_top_table$diffexpressed[Esophageal_cancer_top_table$adj.P.Val< 0.05 & Esophageal_cancer_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Esophageal_cancer_top_table %>% 
...
volcano_plot <- ggplot(data=Esophageal_cancer_top_table, 
  ...
  labs(title="Differential Gene Expression in Esophageal cancer",
       x="log2 Fold Change",
...
print(volcano_plot)
Esophageal_cancer_DEGs <- Esophageal_cancer_top_table[which(abs(Esophageal_cancer_top_table$logFC) > 1 & Esophageal_cancer_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Esophageal_cancer_DEGs)
DEG23 <- unique(Esophageal_cancer_DEGs$Gene.symbol[!is.na(Esophageal_cancer_DEGs$Gene.symbol) & Esophageal_cancer_DEGs$Gene.symbol != ""])
write.csv(DEG23, 
          file = "Esophageal_cancer_DEGs_988个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE9982', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1Esophagealcancer02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Esophagealcancer02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Esophageal cancer临床信息全部22.xlsx')
...
pdf("3Esophagealcancer02Module-trait.pdf",width = 6, height = 12)
...
write.csv(greenyellowGenes, "Esophageal_cancer_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes23 <- intersect(DEG23,greenyellowGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes23)
write.csv(key_genes_df, "Esophageal_cancer_key_genes_98个.csv", row.names = FALSE)