...
library(WGCNA)
#DGE analysis
Tumor_top_table<- read.delim("GSE18105.top.table.tsv", header = TRUE, sep = "\t")
...
write.csv(DEG39, 
          file = "CRC_DEGs_4845个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE18105', destdir=".",AnnotGPL = T,getGPL = T)
...
T1Dsamples <- read_excel('CRC临床信息全部111.xlsx')
...
write.csv(turquoiseGenes, "CRC_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes39<- intersect(DEG39,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes39)
write.csv(key_genes_df, "CRC_key_genes_489个.csv", row.names = FALSE)