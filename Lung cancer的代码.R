...
volcano_plot <- ggplot(data=Lung_cancer_top_table, 
  ...
  labs(title="Differential Gene Expression in Lung cancer",
       x="log2 Fold Change",
...
print(volcano_plot)
Lung_cancer_DEGs <- Lung_cancer_top_table[which(abs(Lung_cancer_top_table$logFC) > 1 & Lung_cancer_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Lung_cancer_DEGs)
DEG04 <- unique(Lung_cancer_DEGs$Gene.symbol[!is.na(Lung_cancer_DEGs$Gene.symbol) & Lung_cancer_DEGs$Gene.symbol != ""])
write.csv(DEG04, 
          file = "Lung_cancer_DEGs_916个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
...
pdf("1Lung02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Lung02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Lung cancer临床信息全部91.xlsx')
...
pdf("3Lung02Module-trait.pdf",width = 6, height = 12)
...
write.csv(blueGenes, "Lung_cancer_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DGE and WGCNA
kGenes04 <- intersect(DEG04,blueGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes04)
write.csv(key_genes_df, "Lung_cancer_key_genes_387个.csv", row.names = FALSE)
