...
library(WGCNA)
#DGE analysis
Hepatoblastoma_top_table<- read.delim("GSE131329.top.table.tsv", header = TRUE, sep = "\t")
Hepatoblastoma_top_table$diffexpressed <- "NS"
Hepatoblastoma_top_table$diffexpressed[Hepatoblastoma_top_table$adj.P.Val< 0.05 & Hepatoblastoma_top_table$logFC > 1] <- "Up"
Hepatoblastoma_top_table$diffexpressed[Hepatoblastoma_top_table$adj.P.Val< 0.05 & Hepatoblastoma_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Hepatoblastoma_top_table %>% 
...
volcano_plot <- ggplot(data=Hepatoblastoma_top_table, 
  ...
  labs(title="Differential Gene Expression in Hepatoblastoma",
       x="log2 Fold Change",
...
Hepatoblastoma_DEGs <- Hepatoblastoma_top_table[which(abs(Hepatoblastoma_top_table$logFC) > 1 & Hepatoblastoma_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Hepatoblastoma_DEGs)
DEG17 <- unique(Hepatoblastoma_DEGs$Gene.symbol[!is.na(Hepatoblastoma_DEGs$Gene.symbol) & Hepatoblastoma_DEGs$Gene.symbol != ""])
write.csv(DEG17, 
          file = "Hepatoblastoma_DEGs_1382个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE131329', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1Hepatoblastoma02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Hepatoblastoma02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Hepatoblastoma临床信息全部67.xlsx')
...
pdf("3Hepatoblastoma02Module-trait.pdf",width = 6, height = 12)
...
write.csv(brownGenes, "Hepatoblastoma_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes17 <- intersect(DEG17,brownGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes17)
write.csv(key_genes_df, "Hepatoblastoma_key_genes_548个.csv", row.names = FALSE)