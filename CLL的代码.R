...
library(WGCNA)
#DGE analysis
CLL_top_table<- read.delim("GSE8835.top.table.tsv", header = TRUE, sep = "\t")
CLL_top_table$diffexpressed <- "NS"
CLL_top_table$diffexpressed[CLL_top_table$adj.P.Val< 0.05 & CLL_top_table$logFC > 1] <- "Up"
CLL_top_table$diffexpressed[CLL_top_table$adj.P.Val< 0.05 & CLL_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  CLL_top_table %>% 
...
volcano_plot <- ggplot(data=CLL_top_table, 
  ...
  labs(title="Differential Gene Expression in CLL",
       x="log2 Fold Change",
...
print(volcano_plot)
CLL_DEGs <- CLL_top_table[which(abs(CLL_top_table$logFC) > 1 & CLL_top_table$adj.P.Val < 0.05), c(2, 7)]
str(CLL_DEGs)
DEG06 <- unique(CLL_DEGs$Gene.symbol[!is.na(CLL_DEGs$Gene.symbol) & CLL_DEGs$Gene.symbol != ""])
write.csv(DEG06, 
          file = "CLL_DEGs_516个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE8835', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1CLL02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2CLL02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('CLL临床信息全部66.xlsx')
...
pdf("3CLL02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "CLL_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
kGenes06 <- intersect(DEG06,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes06)
write.csv(key_genes_df, "CLL_key_genes_250个.csv", row.names = FALSE)