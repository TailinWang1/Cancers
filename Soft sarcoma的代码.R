...
volcano_plot <- ggplot(data=Soft_Sarcoma_top_table, 
  ...
  labs(title="Differential Gene Expression in Soft Sarcoma",
       x="log2 Fold Change",
...
print(volcano_plot)
Soft_Sarcoma_DEGs <- Soft_Sarcoma_top_table[which(abs(Soft_Sarcoma_top_table$logFC) > 1 & Soft_Sarcoma_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Soft_Sarcoma_DEGs)
DEG09 <- unique(Soft_Sarcoma_DEGs$Gene.symbol[!is.na(Soft_Sarcoma_DEGs$Gene.symbol) & Soft_Sarcoma_DEGs$Gene.symbol != ""])
write.csv(DEG09, 
          file = "Soft_Sarcoma_DEGs_2620个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
...
pdf("1SoftSarcoma02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2SoftSarcoma02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('SoftSarcoma临床信息全部54.xlsx')
...
pdf("3SoftSarcoma02Module-trait.pdf",width = 6, height = 12)
...
redGenes <- colnames(red.expr)
write.csv(redGenes, "SoftSarcoma_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes09 <- intersect(DEG09,redGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes09)
write.csv(key_genes_df, "SoftSarcoma_key_genes_186个.csv", row.names = FALSE)