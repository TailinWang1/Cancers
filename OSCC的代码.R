...
library(WGCNA)
#DGE analysis
Tumor_top_table<- read.delim("GSE23558.top.table.tsv", header = TRUE, sep = "\t")
...
str(Tumor_DEGs)
DEG38 <- unique(Tumor_DEGs$Gene.symbol[!is.na(Tumor_DEGs$Gene.symbol) & Tumor_DEGs$Gene.symbol != ""])
write.csv(DEG38, 
          file = "OSCC_DEGs_3377个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE23558', destdir=".",AnnotGPL = T,getGPL = T)
...
T1Dsamples <- read_excel('OSCC临床信息全部32.xlsx')
...
write.csv(blueGenes, "OSCC_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes38<- intersect(DEG38,blueGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes38)
write.csv(key_genes_df, "OSCC_key_genes_825个.csv", row.names = FALSE)
