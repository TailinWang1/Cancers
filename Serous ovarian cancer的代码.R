...
library(WGCNA)
#DGE analysis
Serous_ovarian_cancer_top_table<- read.delim("GSE12470.top.table.tsv", header = TRUE, sep = "\t")
Serous_ovarian_cancer_top_table$diffexpressed <- "NS"
Serous_ovarian_cancer_top_table$diffexpressed[Serous_ovarian_cancer_top_table$adj.P.Val< 0.05 & Serous_ovarian_cancer_top_table$logFC > 1] <- "Up"
Serous_ovarian_cancer_top_table$diffexpressed[Serous_ovarian_cancer_top_table$adj.P.Val< 0.05 & Serous_ovarian_cancer_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Serous_ovarian_cancer_top_table %>% 
...
volcano_plot <- ggplot(data=Serous_ovarian_cancer_top_table, 
  ...
  labs(title="Differential Gene Expression in Serous ovarian cancer",
       x="log2 Fold Change",
...
print(volcano_plot)
Serous_ovarian_cancer_DEGs <- Serous_ovarian_cancer_top_table[which(abs(Serous_ovarian_cancer_top_table$logFC) > 1 & Serous_ovarian_cancer_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Serous_ovarian_cancer_DEGs)
DEG25 <- unique(Serous_ovarian_cancer_DEGs$Gene.symbol[!is.na(Serous_ovarian_cancer_DEGs$Gene.symbol) & Serous_ovarian_cancer_DEGs$Gene.symbol != ""])
write.csv(DEG25, 
          file = "Serous_ovarian_cancer_DEGs_3289个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE12470', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1Serousovariancancer02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Serousovariancancer02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Serous ovarian cancer临床信息全部53.xlsx')
...
pdf("3Serousovariancancer02Module-trait.pdf",width = 6, height = 12)
...
write.csv(blueGenes, "Serous_ovarian_cancer_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes25 <- intersect(DEG25,blueGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes25)
write.csv(key_genes_df, "Serous_ovarian_cancer_key_genes_615个.csv", row.names = FALSE)
