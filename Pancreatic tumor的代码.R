...
library(WGCNA)
#DGE analysis
Pancreatic_tumor_top_table<- read.delim("GSE43795.top.table.tsv", header = TRUE, sep = "\t")
Pancreatic_tumor_top_table$diffexpressed <- "NS"
Pancreatic_tumor_top_table$diffexpressed[Pancreatic_tumor_top_table$adj.P.Val< 0.05 & Pancreatic_tumor_top_table$logFC > 1] <- "Up"
Pancreatic_tumor_top_table$diffexpressed[Pancreatic_tumor_top_table$adj.P.Val< 0.05 & Pancreatic_tumor_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Pancreatic_tumor_top_table %>% 
...
volcano_plot <- ggplot(data=Pancreatic_tumor_top_table, 
  ...
  labs(title="Differential Gene Expression in Pancreatic tumor",
       x="log2 Fold Change",
...
print(volcano_plot)
Pancreatic_tumor_DEGs <- Pancreatic_tumor_top_table[which(abs(Pancreatic_tumor_top_table$logFC) > 1 & Pancreatic_tumor_top_table$adj.P.Val < 0.05), c(2, 8)]
str(Pancreatic_tumor_DEGs)
DEG28 <- unique(Pancreatic_tumor_DEGs$Gene.symbol[!is.na(Pancreatic_tumor_DEGs$Gene.symbol) & Pancreatic_tumor_DEGs$Gene.symbol != ""])
write.csv(DEG28, 
          file = "Pancreatic_tumor_DEGs_2586个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE43795', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1Pancreatictumor02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Pancreatictumor02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Pancreatic tumor临床信息全部31.xlsx')
...
pdf("3Pancreatictumor02Module-trait.pdf",width = 6, height = 12)
...
write.csv(brownGenes, "Pancreatic_tumor_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes28 <- intersect(DEG28,brownGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes28)
write.csv(key_genes_df, "Pancreatic_tumor_key_genes_525个.csv", row.names = FALSE)
