...
library(WGCNA)
#DGE analysis
Prostate_cancer_top_table<- read.delim("GSE8511.top.table.tsv", header = TRUE, sep = "\t")
Prostate_cancer_top_table$diffexpressed <- "NS"
Prostate_cancer_top_table$diffexpressed[Prostate_cancer_top_table$adj.P.Val< 0.05 & Prostate_cancer_top_table$logFC > 1] <- "Up"
Prostate_cancer_top_table$diffexpressed[Prostate_cancer_top_table$adj.P.Val< 0.05 & Prostate_cancer_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Prostate_cancer_top_table %>% 
...
volcano_plot <- ggplot(data=Prostate_cancer_top_table, 
  ...
  labs(title="Differential Gene Expression in Prostate cancer",
       x="log2 Fold Change",
...
print(volcano_plot)
Prostate_cancer_DEGs <- Prostate_cancer_top_table[which(abs(Prostate_cancer_top_table$logFC) > 1 & Prostate_cancer_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Prostate_cancer_DEGs)
DEG29 <- unique(Prostate_cancer_DEGs$Gene.symbol[!is.na(Prostate_cancer_DEGs$Gene.symbol) & Prostate_cancer_DEGs$Gene.symbol != ""])
write.csv(DEG29, 
          file = "Prostate_cancer_DEGs_12个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE8511', destdir=".",AnnotGPL = T,getGPL = T)
...
var_cutoff = quantile(m.vars, 0.90) 
expro.upper = datExpr0[, which(m.vars > var_cutoff)]
...
pdf("1Prostatecancer02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Prostatecancer02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Prostate cancer临床信息全部41.xlsx')
...
pdf("3Prostatecancer02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "Prostate_cancer_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes29 <- intersect(DEG29,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes29)
write.csv(key_genes_df, "Prostate_cancer_key_genes_11个.csv", row.names = FALSE)
