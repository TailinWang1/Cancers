...
top_genes <- rbind(
  Acute_myeloid_leukemia_top_table %>% 
...
volcano_plot <- ggplot(data=Acute_myeloid_leukemia_top_table, 
  ...
  labs(title="Differential Gene Expression in AML",
       x="log2 Fold Change",
...
print(volcano_plot)
Acute_myeloid_leukemia_DEGs <- Acute_myeloid_leukemia_top_table[which(abs(Acute_myeloid_leukemia_top_table$logFC) > 1 & Acute_myeloid_leukemia_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Acute_myeloid_leukemia_DEGs)
DEG01 <- unique(Acute_myeloid_leukemia_DEGs$Gene.symbol[!is.na(Acute_myeloid_leukemia_DEGs$Gene.symbol) & Acute_myeloid_leukemia_DEGs$Gene.symbol != ""])
write.csv(DEG01, 
          file = "AML_DEGs_470个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA analysis
...
pdf("1AML02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2AML02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('AML临床信息全部64.xlsx')
...
pdf("3AML02Module-trait.pdf",width = 6, height = 12)
...
write.csv(blueGenes, "AML_WGVNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DGE and WGCNA
kGenes01 <- intersect(DEG01,blueGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes01)
write.csv(key_genes_df, "AML_key_genes_151个.csv", row.names = FALSE)
