...
library(WGCNA)
#DGE analysis
Tumor_top_table<- read.delim("GSE132550.top.table.tsv", header = TRUE, sep = "\t")
...
str(Tumor_DEGs)
DEG39 <- unique(Tumor_DEGs$ORF[!is.na(Tumor_DEGs$ORF) & Tumor_DEGs$ORF != ""])
write.csv(DEG39, 
          file = "PTCL_DEGs_10118个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE132550', destdir=".",AnnotGPL = T,getGPL = T)
...
T1Dsamples <- read_excel('PTCL临床信息全部90.xlsx')
...
write.csv(blueGenes, "PTCL_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes39<- intersect(DEG39,blueGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes39)
write.csv(key_genes_df, "PTCL_key_genes_172个.csv", row.names = FALSE)
