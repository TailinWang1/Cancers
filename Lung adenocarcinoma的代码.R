...
library(WGCNA)
#DGE analysis
Lung_adenocarcinoma_top_table<- read.delim("GSE10072.top.table.tsv", header = TRUE, sep = "\t")
Lung_adenocarcinoma_top_table$diffexpressed <- "NS"
Lung_adenocarcinoma_top_table$diffexpressed[Lung_adenocarcinoma_top_table$adj.P.Val< 0.05 & Lung_adenocarcinoma_top_table$logFC > 1] <- "Up"
Lung_adenocarcinoma_top_table$diffexpressed[Lung_adenocarcinoma_top_table$adj.P.Val< 0.05 & Lung_adenocarcinoma_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Lung_adenocarcinoma_top_table %>% 
...
volcano_plot <- ggplot(data=Lung_adenocarcinoma_top_table, 
  ...
  labs(title="Differential Gene Expression in Lung adenocarcinoma",
       x="log2 Fold Change",
...
print(volcano_plot)
Lung_adenocarcinoma_DEGs <- Lung_adenocarcinoma_top_table[which(abs(Lung_adenocarcinoma_top_table$logFC) > 1 & Lung_adenocarcinoma_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Lung_adenocarcinoma_DEGs)
DEG34 <- unique(Lung_adenocarcinoma_DEGs$Gene.symbol[!is.na(Lung_adenocarcinoma_DEGs$Gene.symbol) & Lung_adenocarcinoma_DEGs$Gene.symbol != ""])
write.csv(DEG34, 
          file = "Lung_adenocarcinoma_DEGs_662个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE10072', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1Lungadenocarcinoma02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Lungadenocarcinoma02module.pdf",width = 12, height = 5)
...
#表型与模块的相关性
# 首先将 Condition 列转换为因子
T1Dsamples <- read_excel('Lung adenocarcinoma临床信息全部107.xlsx')
# 保存第一列作为行名
row_names <- T1Dsamples[[1]]
# 使用 select() 从 tibble 移除第一列
...
MEsWW = orderMEs(MEs0)
dim(MEsWW)   # 查看 MEsWW 的维度
dim(T1Dsamples_df) # 查看 RAdesign 的维度
...
pdf("3Lungadenocarcinoma02Module-trait.pdf",width = 6, height = 12)
...
write.csv(turquoiseGenes, "Lung_adenocarcinoma_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes34<- intersect(DEG34,turquoiseGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes34)
write.csv(key_genes_df, "Lung_adenocarcinoma_key_genes_468个.csv", row.names = FALSE)