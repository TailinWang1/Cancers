...
library(WGCNA)
#DGE analysis
Kidney_cancer_top_table<- read.delim("GSE15641.top.table.tsv", header = TRUE, sep = "\t")
Kidney_cancer_top_table$diffexpressed <- "NS"
Kidney_cancer_top_table$diffexpressed[Kidney_cancer_top_table$adj.P.Val< 0.05 & Kidney_cancer_top_table$logFC > 1] <- "Up"
Kidney_cancer_top_table$diffexpressed[Kidney_cancer_top_table$adj.P.Val< 0.05 & Kidney_cancer_top_table$logFC< -1] <- "Down"
top_genes <- rbind(
  Kidney_cancer_top_table %>% 
...
volcano_plot <- ggplot(data=Kidney_cancer_top_table, 
  ...
  labs(title="Differential Gene Expression in Kidney cancer",
       x="log2 Fold Change",
...
print(volcano_plot)
Kidney_cancer_DEGs <- Kidney_cancer_top_table[which(abs(Kidney_cancer_top_table$logFC) > 1 & Kidney_cancer_top_table$adj.P.Val < 0.05), c(2, 7)]
str(Kidney_cancer_DEGs)
DEG15 <- unique(Kidney_cancer_DEGs$Gene.symbol[!is.na(Kidney_cancer_DEGs$Gene.symbol) & Kidney_cancer_DEGs$Gene.symbol != ""])
write.csv(DEG15, 
          file = "Kidney_cancer_DEGs_1330个.csv", 
          row.names = FALSE,
          quote = FALSE)
#WGCNA
T1D_gset <- getGEO('GSE15641', destdir=".",AnnotGPL = T,getGPL = T)
...
pdf("1Kidneycancer02Threshold.pdf",width = 10, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf("2Kidneycancer02module.pdf",width = 12, height = 5)
...
T1Dsamples <- read_excel('Kidney cancer临床信息全部92.xlsx')
...
pdf("3Kidneycancer02Module-trait.pdf",width = 6, height = 12)
...
yellowGenes <- colnames(yellow.expr)
write.csv(yellowGenes, "Kidney_cancer_WGCNA最相关基因.csv", row.names = FALSE, quote = FALSE)
#Combine DEG and WGCNA
kGenes15 <- intersect(DEG15,yellowGenes)
key_genes_df <- data.frame(Gene_Symbol = kGenes15)
write.csv(key_genes_df, "Kidney_cancer_key_genes_103个.csv", row.names = FALSE)